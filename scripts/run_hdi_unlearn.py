"""
scripts/run_hdi_unlearn.py
==========================
Run Holographic Destructive Interference (HDI) Unlearning on TOFU.

Usage
-----
  python scripts/run_hdi_unlearn.py --debug
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="ARMOR Vanguard — HDI Unlearning")
    p.add_argument("--debug",      action="store_true")
    p.add_argument("--model",      default="debug", choices=["debug","mistral-7b","llama2-7b"])
    p.add_argument("--qlora",      action="store_true")
    p.add_argument("--hf-token",   default=None)
    p.add_argument("--output-dir", default="outputs/hdi")
...
def main():
    ...
    print("=" * 60)
    print(f"  ARMOR Vanguard — Holographic Destructive Interference (HDI)")
    ...
    # ── Zero-Shot Unlearning via HDI ──────────────────────────────────────────
    hdi = HolographicInterference(model, cfg)
    hdi.unlearn(eval_forget_loader, eval_retain_loader)
    ...
    if not args.no_save:
        ...
        ckpt_path = os.path.join(args.output_dir, "hdi_unlearned")
